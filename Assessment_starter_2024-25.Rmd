...
gt_medicine_table = medicine_table %>% 
  gt()
...
```{r}
health_board <- read_csv("https://www.opendata.nhs.scot/dataset/9f942fdb-e59e-44f5-b534-d6e17229cc7b/resource/652ff726-e676-4a20-abda-435b98dd7bdc/download/hb14_hb19.csv") %>% 
  clean_names()
  
joined_data = full_join(health_board, data_march2024,join_by(hb == hbt))
...
```{r}
Scottish_Ambulance_Service = joined_data[is.na(joined_data$hb_name), ]
```
...
```{r}
joined_data_2 = full_join(population_data,joined_data,join_by(hb_name))
```
...
```{r}
paracetamol_data <- joined_data_2 %>% 
...
```{r}
new_paracetamol_data <-  joined_data_2 %>% 
  filter(str_detect(bnf_item_description, "PARACET"),
         grepl('CAPSULES|CAPLETS', 
               bnf_item_description)) %>% 
   group_by(bnf_item_description, hb_name, hb_population) %>% 
  summarise(total_prescriptions = sum(paid_quantity)) %>% 
  mutate("per_person" = total_prescriptions/hb_population)

paracetamol_graph = new_paracetamol_data %>% 
  ggplot(aes(x= reorder(hb_name, -per_person), y = per_person))+
  geom_col() + 
  coord_flip()
  
 
print(paracetamol_graph)
...
```{r}
covid_data <-  pitc202101 %>% 
  clean_names() %>% 
  full_join(health_board, pitc202101,
            join_by(HBT=hb))
#wrong ⬆️
  filter(str_detect(bnf_item_description, "PARACET"),
         grepl('CAPSULES|CAPLETS', 
